...
class SQL {
    ...
    static async addEmployee() {
        ...
        let listOfRoles = roles.map(role => role.title);
        let listOfEmployees = employees.map(employee => employee.name);
        listOfEmployees.push("No Manager"); // Adding 'No Manager' as an option
        const result = await inquirer.prompt([
            {
                type: "input",
        ...
        const employeeID = (result.employeeManager === "No Manager") ? null : employees.filter(employee => employee.name === result.employeeManager)[0].id;
        await connection.promise().query(`
    ...
    static async deleteEmployee() { // *** Would be great to add a doble validation to ensure users do not accidentally delete a department in future versions ***
        ...
        try {
            ...
            console.log(`Employee "${result.employeeName}" cannot be deleted! Please ensure you do not have managers assigned to this employee before deleting!\n`);
        }
    };
